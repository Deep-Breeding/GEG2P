...
class SpatialGatingUnit(nn.Module):
    def __init__(
        ...
        self.heads = heads
        self.causal = causal
        self.norm = nn.LayerNorm(dim_out)  # 512*2
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [Residual(
                PreNorm(
                    dim, PreShiftTokens(
                        token_shifts, gMLPBlock(
                            dim=dim, heads=heads, dim_ff=dim_ff,
                            seq_len=seq_len, attn_dim=attn_dim, causal=causal,
                            act=act, circulant_matrix=circulant_matrix)
                    )
                )
            )
...
class gMLPVision(nn.Module):
    def __init__(
        ...
        super().__init__()
        assert (dim % heads) == 0, 'dimension must be divisible by number of heads'
        seq_len = seq_len  # 30000,1
        num_patches = num_patches  # 800
        dim_ff = dim * ff_mult

        self.to_patch_embed = nn.Sequential(
            nn.Linear(seq_len, dim)  # 512
        )
        # [batch,1,dim]
        self.prob_survival = prob_survival
        self.layers = nn.ModuleList([Residual(
            PreNorm(dim, gMLPBlock(dim=dim, heads=heads, dim_ff=dim_ff, seq_len=num_patches, attn_dim=attn_dim))) for i
                                     in range(depth)])
        self.to_logits = nn.Sequential(
            nn.LayerNorm(dim),
    ...
    def forward(self, x):
        # print('x.shape_before_patch_embed:',x.shape) # [24, 1, 32062, 1]
        # print(x.shape)
        x = self.to_patch_embed(x)
        # print(x.shape)
        # print('x1.shape_after_patch_embed:',x.shape) # [24, 1, 512]
        layers = self.layers if not self.training else dropout_layers(self.layers, self.prob_survival)
