...
def get_dataloader(snp_path, phe_path, cvf_path,  area):
    ...
    with open(snp_path) as file:
        id = ''
        for line in islice(file, 1, None):  # Iterate through each line in the file starting from the second line (index 1)
    ...
    list_phe = list_phe.to_numpy()  # DataFrame 转换为 NumPy 数组。
    list_phe = list_phe.reshape(-1, 1)  # 转换为一个二维数组，其中的列数为 1
