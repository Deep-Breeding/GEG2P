...
# Add current directory to path for importing utils modules
...
def create_objective(model_class, trait,snp_path, phe_path, cvf_path,device,num_workers,snp_num, kmax=10):
    def objective(trial):
        ...
        train_loader, val_loader, _ = DL_dataloader(snp_path, phe_path, cvf_path, batch_size, trait,1,num_workers, kmax)
...
def DL_dataloader(snp_path, phe_path, cvf_path, batch_size, area, k, num_workers, kmax=10):
    list_id = []
    dictsnp = {}
...
def train_DL(plant,traits,model_classes,snp_path, phe_path, cvf_path,device,num_workers,kmax, snp_num):
    for trait in traits:
        ...
        for model_name, model_class in model_classes.items():
            ...
            study.optimize(create_objective(model_class, trait,snp_path, phe_path, cvf_path,device,num_workers,snp_num, kmax), n_trials=50)
            ...
            for k in range(1, kmax+1):
                ...
                train_loader, val_loader, test_loader = DL_dataloader(snp_path, phe_path, cvf_path,
                                                                      batch_size, trait,k,num_workers, kmax)
